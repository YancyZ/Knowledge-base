...
每个新项目若从零配置 Vite、ESLint、TS、测试、CI，首周往往耗在「搭环境」而非业务。脚手架把**已验证的最佳实践**固化成模板。
...
- [ ] Git 初始化，配置 `.gitignore`
- [ ] 选定 Node 版本（`.nvmrc` + `engines`）
- [ ] 统一包管理器（`packageManager` 字段）
...
脚手架缩短项目冷启动时间，Monorepo 与 Changesets 支撑多包协作与版本发布。选型时在「单体 + 路由懒加载」与「微前端 / 独立部署」之间权衡，避免过早引入不必要的复杂度。
