...
本系列提供框架专属规范（见 [前端框架篇](../前端框架篇/)）：

- [React 编码规范](../前端框架篇/React/React编码规范.md)
- [Vue 编码规范](../前端框架篇/Vue/Vue编码规范.md)
...
- [ ] ESLint + Prettier + EditorConfig 三件套
- [ ] Husky pre-commit + lint-staged
- [ ] Commitlint 规范提交
- [ ] `strict` TypeScript + CI typecheck
- [ ] Vitest 覆盖核心逻辑
- [ ] CI 门禁：lint + typecheck + test 全绿才合并
- [ ] 团队编码规范文档化并 Review 对照
- [ ] 启用 type-aware ESLint 规则（no-floating-promises 等）
- [ ] dependency-cruiser 禁止循环依赖与跨层 import
- [ ] MSW `onUnhandledRequest: 'error'`
- [ ] CI bundle size / Lighthouse 预算

---

## 十八、小结

质量保障依赖 ESLint、Prettier、TypeScript 与测试的分工协作，Git 钩子与 CI 门禁将规范自动化。type-aware lint 与架构依赖约束把「约定」变成可执行的规则，性能预算则把体验指标纳入交付标准。
