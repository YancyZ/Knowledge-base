...
> **目标**：识别常见 Web 威胁，在开发、构建、部署全链路落地可执行的防护方案。
...
## 九、Content-Security-Policy 完整实践
...
前端安全需要理解 **XSS 三类机理**（存储、反射、DOM）及 mXSS 等变体，并落实 **输出编码、输入校验、消毒、CSP、凭证保护** 多层防御。CSRF 依赖 **SameSite、CSRF Token、Origin 校验、禁止 GET 改状态** 组合，不能误解 CORS 可替代 CSRF 防护。配合供应链审计、安全响应头与事件流程，才能将安全从「知道概念」变为「可执行工程能力」。
