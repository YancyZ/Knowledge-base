...
### 8.6 React Compiler — 构建期自动记忆化

**React Compiler**（`babel-plugin-react-compiler`）在编译阶段分析组件数据流，自动插入与 `memo` / `useMemo` / `useCallback` 等价的优化，不改变运行时语义。

| 维度 | 说明 |
|------|------|
| 形态 | Babel 插件（非运行时） |
| 目标 | 减少手写性能 Hook 与 deps 错误 |
| 不替代 | 虚拟列表、代码分割、状态下沉 |
| 启用 | Babel 配置或 Next.js `experimental.reactCompiler` |
...
Vite 项目若用 `@vitejs/plugin-react`（Babel 路径），可在插件选项中注入 Compiler；若用 SWC 插件则需关注官方对 Compiler 的支持进度。

**与 SWC / esbuild 关系**：Compiler 当前走 Babel AST；Next.js 15+ 集成 SWC + Compiler 由框架封装。纯 esbuild 转译路径**不会**自动启用 Compiler。

框架侧输出对比与 opt-out 见 [React Compiler 概览](../前端框架篇/React/18-React19与新特性/03-React-Compiler概览.md)；React/Vue 工程化对照见 [14-框架工程化实践](./14-框架工程化实践.md)。

### 8.7 Vue SFC 编译（@vitejs/plugin-vue）

| 块 | 编译产物 |
|----|----------|
| `<template>` | render 函数（运行时或编译时） |
| `<script>` | 标准 JS/TS 模块 |
| `<style scoped>` | 带 data-v-hash 的选择器 |

与 React JSX 不同，Vue SFC 由 **plugin-vue** 在 dev/build 时拆块编译，HMR 以 `.vue` 文件为粒度。
