...
## single-spa 与 Rspack Federation
...
微前端解决**多团队独立交付**同一站点：路由级/组件级集成，模块联邦共享运行时依赖，须提前约定版本与样式隔离。

qiankun 沙箱 + 路由分发；Module Federation shared 对齐 react 等 singleton；CSS 隔离用 shadow 或 prefix；独立部署 + 回滚。

**易混点**：双份 React；全局样式污染；子应用路由与基座冲突；过度微前端增加复杂度。

核对：shared 依赖版本是否 lock？子应用独立 CI 是否绿？
