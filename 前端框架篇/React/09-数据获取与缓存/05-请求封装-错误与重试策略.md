...
HTTP 层（baseURL、鉴权、错误类型）与 Query 层（cache、retry、abort）应分开：**request 封装**管怎么打 API，**TanStack Query** 管何时打、打几次、怎么缓存；分层架构、401 处理、retry 策略与 AbortSignal。
...
**request 层**：baseURL、鉴权头、统一错误类型、401 处理；**Query 层**：cache、retry、abort。**UI 层**：局部 ErrorBoundary + 重试按钮；全局 toast 报网络/业务错误。

Query **retry** 配指数退避；4xx 一般不重试，POST 支付**不要**自动 retry。queryFn 传 **AbortSignal** 防竞态；类型可与 OpenAPI 生成对齐。

常见错因：401 是否在 request 层统一处理？4xx 是否被错误 retry？敏感错误是否泄露 stack 给用户？
