# 动画与手势

React 里做动画：**CSS 优先**；复杂序列用 **Motion（Framer Motion）**；列表重排可用 **View Transitions**；手势与拖拽有专用库。原则：**不阻塞主线程、尊重 prefers-reduced-motion**。

---

## 选型地图

```mermaid
flowchart TD
  Start[需要动画?]
  Start --> Q1{仅 hover/过渡?}
  Q1 -->|是| CSS[CSS transition / animation]
  Q1 -->|否| Q2{组件进出场?}
  Q2 -->|是| Motion[Motion / CSS]
  Q2 -->|否| Q3{拖拽/手势?}
  Q3 -->|是| DnD[dnd-kit / use-gesture]
```

| 方案 | 场景 |
|------|------|
| **CSS** | 颜色、opacity、简单位移 |
| **Motion** | Modal、页面过渡、layout 动画 |
| **react-spring** | 物理弹簧 |
| **@dnd-kit** | 拖拽排序 |
| **View Transitions API** | 路由/主题切换（浏览器） |

---

## CSS 动画（首选）

```tsx
function Card({ visible }: { visible: boolean }) {
  return (
    <div className={visible ? 'card card--in' : 'card card--out'}>
      ...
    </div>
  );
}
```

```css
.card {
  transition: opacity 0.2s, transform 0.2s;
}
.card--out {
  opacity: 0;
  transform: translateY(8px);
}
```

无额外 JS、GPU 友好，简单过渡首选 CSS。

---

## Motion 基础

```bash
pnpm add motion
```

```tsx
import { motion, AnimatePresence } from 'motion/react';

function Modal({ open, children }: Props) {
  return (
    <AnimatePresence>
      {open && (
        <motion.div
          initial={{ opacity: 0, scale: 0.95 }}
          animate={{ opacity: 1, scale: 1 }}
          exit={{ opacity: 0, scale: 0.95 }}
          transition={{ duration: 0.2 }}
        >
          {children}
        </motion.div>
      )}
    </AnimatePresence>
  );
}
```

| 概念 | 说明 |
|------|------|
| `AnimatePresence` | 退出动画需包裹 |
| `layout` | 列表重排平滑 |

---

## 与 React 状态

动画跟 **state 走**，勿直接操作 DOM：

```tsx
const [open, setOpen] = useState(false);
// ✅ state → motion animate
// ❌ element.style.transform = ...
```

列表动画加 **稳定 key**。

---

## 拖拽 @dnd-kit

```tsx
import { DndContext, closestCenter } from '@dnd-kit/core';
import { SortableContext, useSortable, arrayMove } from '@dnd-kit/sortable';

// 与 React state 同步 items 顺序
function onDragEnd(event) {
  const { active, over } = event;
  if (over && active.id !== over.id) {
    setItems(items => arrayMove(items, oldIndex, newIndex));
  }
}
```

拖拽排序用 @dnd-kit，与 React state 同步 items 顺序。

---

## 无障碍

```css
@media (prefers-reduced-motion: reduce) {
  *, *::before, *::after {
    animation-duration: 0.01ms !important;
    transition-duration: 0.01ms !important;
  }
}
```

Motion：`useReducedMotion()` 缩短或关闭动画。

---

## 性能

| ❌ | ✅ |
|----|-----|
| 动画 width/height top | transform、opacity |
| 长列表每项复杂 motion | 仅可见项或 CSS |
| 与 Concurrent 抢 | `startTransition` 包非紧急 |

---

## 小结

CSS 优先，复杂序列用 Motion，拖拽用 dnd-kit；尊重 prefers-reduced-motion。

动画选型：简单过渡→CSS；进出场/Modal→Motion + AnimatePresence；拖拽→@dnd-kit；路由切换→View Transitions API。动画由 state 驱动，不直接改 DOM；列表动画用稳定 key。无障碍：prefers-reduced-motion 缩短动画，Motion 用 useReducedMotion()。性能：动画 transform/opacity 而非 width/height；长列表避免每项复杂 motion；非紧急动画用 startTransition。
