...
> 在 JavaScript 之上增加**编译期类型**，擦除后仍是 JS。运行时模型见 **03 · JavaScript**。

---

## 一、TypeScript 是什么：特性与发展
...
**类型断言**告诉编译器「我比类型检查器更确定这个值的类型」，**不产生运行时检查**。
...
```mermaid
flowchart TB
  Base[基础类型<br/>原始 · 数组 · 元组 · enum]
  Object[对象建模<br/>对象 · interface · class · type]
  Logic[类型逻辑<br/>联合 · 交叉 · 泛型 · 条件 · 映射]
  Safety[安全机制<br/>推论 · 断言 · 守卫 · 兼容]
  Eng[工程能力<br/>模块 · 工具类型 · tsconfig · 装饰器]
  Base --> Object --> Logic --> Safety --> Eng
```



TypeScript = **JS 语义** + **编译期契约**：


| 层次    | 手段                        |
| ----- | ------------------------- |
| 建模数据  | 原始类型、对象、interface、可辨识联合   |
| 建模复用  | 泛型、工具类型、映射/条件类型           |
| 建模分支  | 联合 + 类型收窄 + `never` 穷尽    |
| 建模工程  | 模块、`.d.ts`、`tsconfig`、装饰器 |
| 运行时安全 | zod 等校验，不能单靠 TS           |


会写 TS 仍须懂 **03 篇** 的运行时、异步与事件循环——类型不能替代对 JavaScript 本身的理解。