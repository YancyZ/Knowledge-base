...
> 标记语言：用标签描述**结构与含义**，是页面的骨架与语义基础。

---

## 一、HTML 在页面中的角色

浏览器读取 HTML 字节流，解析为**文档树**，再与样式表合成可见页面。无论使用何种开发方式，最终都要产出**合法、语义正确的 HTML**（或与之等价的文档结构）。
...
HTML 决定**页面结构是否清晰、语义是否正确、能否被搜索引擎与辅助技术理解**。选用合适标签、写好 head 元数据、保持文档大纲与 SEO 结构化数据一致，是后续样式与脚本工作的前提。写模板时去掉样式类名，结构本身仍应是一篇合格文档。
