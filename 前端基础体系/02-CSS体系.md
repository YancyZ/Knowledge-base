...
见 **7.3**；`perspective`、`transform-style: preserve-3d` 可做简单 3D 翻转卡片。
...
CSS 体系 = **层叠谁赢** + **普通流/定位/浮动/Flex/Grid 如何排布** + **盒模型与 offset/client/scroll 尺寸** + **过渡动画、transform 与 GPU 合成层** + **媒体查询与 CSS3 模块能力**。改样式前先判断会不会触发布局；动画优先 transform/opacity；移动端注意 **0.5px**、**dvh** 与**堆叠上下文**；勿滥用 `translateZ(0)` 与 `will-change`。
