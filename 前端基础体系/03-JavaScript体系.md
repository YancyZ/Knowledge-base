...
> 浏览器中的脚本语言：类型与运行时、函数与对象、异步与事件循环、DOM 与事件、语言演进（ES5–ES2026）。

---

## 一、运行时：堆、栈与队列

JavaScript 在浏览器**主线程**执行：同一时刻只有一个**调用栈**在跑同步代码。理解「栈上执行、堆上存对象、任务进队列」，是读懂异步与性能问题的前提。
...
## 十六、异步：回调地狱、Promise 与 async/await
...
JavaScript 体系 = **栈堆与任务队列** + **原始/引用类型与转换** + **控制流与 try/catch** + **对象操作** + **作用域 / 闭包** + **原型链** + **数组与拷贝** + **浏览器（DOM、回流/重绘、fetch/Storage/History、事件）** + **回调地狱 → Promise → 事件循环** + **ES 逐年演进**。

写业务时：默认 `===` 与 `??`；引用类型注意共享与浅拷；回调注意 `this` 与闭包；高频事件防抖节流；DOM 批量读写防回流、动画优先 `transform`；异步用 `async/await` + `Promise.all` 并行；`fetch` 须查 `res.ok`；敏感数据勿放 `localStorage`。
